...
Vec2d* path(Vec2d start, Entity** entities, GameState* gState) {
    ...
    Vec2i* queue = calloc(gState->currentLevel->currentRoom->width * gState->currentLevel->currentRoom->height, sizeof(Vec2i));
    int qFront = 0, qEnd = 0;
    queue[0] = startTile;
    int** bfs = calloc(gState->currentLevel->currentRoom->width, sizeof(int*));
    for(int i = 0; i < gState->currentLevel->currentRoom->width; i++) {
        bfs[i] = calloc(gState->currentLevel->currentRoom->height, sizeof(int));
    }
    int MAX_SIZE = gState->currentLevel->currentRoom->width * gState->currentLevel->currentRoom->height;
